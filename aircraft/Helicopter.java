...
public class Helicopter extends Aircraft implements Flyable{
	...
	public void updateConditions() {
		...
		if (weather.equals("SUN")) {
			...
			Printer.addLine("Helicopter#" + this.name + "(" + this.id + "): is zooming through the skies! Now where is my promised minigun?");
		} else if (weather.equals("RAIN")) {
			this.coordinates = new Coordinates(coordinates.getLongitude() + 5, coordinates.getLatitude(), coordinates.getHeight());
			Printer.addLine("Helicopter#" + this.name + "(" + this.id + "): is not to have the side doors left open for wild yeehaws.");
		} else if (weather.equals("FOG")) {
			this.coordinates = new Coordinates(coordinates.getLongitude() + 1, coordinates.getLatitude(), coordinates.getHeight());
			Printer.addLine("Helicopter#" + this.name + "(" + this.id + "): is possibly upside down. Proooobably fine. Probably.");
		} else if (weather.equals("SNOW")) {
			this.coordinates = new Coordinates(coordinates.getLongitude(), coordinates.getLatitude(), coordinates.getHeight() - 12);
			Printer.addLine("Helicopter#" + this.name + "(" + this.id + "): has very fast spinny thing on top. Like an ice beyblade");
		} else {
        ...
        {
			Printer.addLine("Helicopter#" + this.name + "(" + this.id + "): has safely landed at " + 
								getLandingCoordinates(coordinates) +
								". Now to go stick my hand into the spinny thing at the back");
            this.weatherTower.unregister(this);
            Printer.addLine("Tower says: Helicopter#" + this.name + "(" + this.id + ") unregistered from weather tower. Only one hand lost");
        }
	}
